...
class TransparentProxy:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        if client_ip == blocked_ip:
            logging.info(
                f"Blocked request from {client_ip} to {flow.request.pretty_url}"
            )
            flow.response = http.HTTPResponse.make(403, b"Forbidden", {"Content-Type": "text/plain"})
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        logging.info(f"Received response from {flow.request.pretty_url} for {client_ip} (status code: {flow.response.status_code})")
